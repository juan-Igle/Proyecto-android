...
import com.example.emergitech.presentation.ui.theme.OnPrimary
    ...
    TopAppBar(
        title = {
            Text(
                text = title,
                fontSize = 19.sp,
                color = OnPrimary // Manteniendo el texto en blanco para un buen contraste
            )
        },
            ...
            if (upAvailable) {
                IconButton(onClick = { navController?.popBackStack() }) {
                    Icon(
                        ...
                        contentDescription = "",
                        tint = OnPrimary // Manteniendo el icono en blanco para visibilidad
                    )
                }
            }
        }
